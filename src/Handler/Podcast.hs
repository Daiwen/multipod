...
extractInfos podcast =
  case podcast of
    Just (Entity idPod _) -> do
      ...
      return $
        map
          (\(Entity idEp e) -> (idEp, pack $ episodeName e, pack $ episodeUrl e, episodeIsRead e))
          episodes
    Nothing -> return []
...
episodeListField b titles =
  Field
  { fieldParse = \rawVals _fileVals -> return $ Right $ Just $ map read rawVals
...
handlePodcastResult res = do
  ...
  let actionValue = case action of
                Just   "read"    -> Just True
  ...
  case (res, actionValue) of
    ...
    (FormSuccess (Just ids), Just value) -> do _ <- sequence $ map (runDB . (updateEpisodeIsRead value)) ids; return False
    _ -> return False
